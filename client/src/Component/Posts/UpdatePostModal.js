...
const UpdatePostModal = forwardRef((props, forwardedRef) => {
    const {
        postState: { post },
    ...
    const [updateNewPost, setUpdateNewPost] = useState(post);
    const { title, description, url, status } = updateNewPost;

    useEffect(() => setUpdateNewPost(post), [post]);

    const submitUpdatePost = async (e) => {
        try {
            e.preventDefault();
            const { success } = await updatePost(updateNewPost);
            setShowUpdatePostModal(false);
            success ? toast.success('Update Success') : toast.error('Update Failure');
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const onChangeUpdatePost = (e) => {
        setUpdateNewPost({ ...updateNewPost, [e.target.name]: e.target.value });
    };
    ...
    return (
        <>
            <div
                tabIndex="-1"
                className={`${
                    showUpdatePostModal ? '' : 'hidden'
                } fixed flex justify-center items-center bg-black/[0.3] left-0 right-0 z-[100] w-full h-full p-4 overflow-x-hidden overflow-y-auto md:inset-0  max-h-full`}
            >
                <Transition show={showUpdatePostModal} as={Fragment}>
                    <Dialog
                        onClose={closeModal}
                        className="fixed flex justify-center items-center inset-0 z-[101] overflow-y-auto"
                    >
                            ...
                            >
                                <div ref={forwardedRef} className="relative w-full max-w-xl min-w-[470px] max-h-full">
                                    <div className="relative bg-white rounded-lg shadow">
                                        ...
                                        <form onSubmit={submitUpdatePost} className="flex m-6 items-center flex-col">
                                            <input
                                                type="text"
                                                name="title"
                                                value={title}
